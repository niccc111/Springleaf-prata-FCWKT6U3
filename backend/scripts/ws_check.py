...
Opens the dispatcher event stream, triggers a re-optimisation, a route lock and
a priority order, then asserts the expected events arrive.
...
async def main(base_url: str) -> int:
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=180) as c:
        ...
        async with websockets.connect(ws_url) as ws:
            ...
            await c.post("/api/v1/optimise", json={"reoptimise": True})
            routes = (await c.get("/api/v1/routes")).json()
            if routes:
                await c.patch(f"/api/v1/routes/{routes[0]['route_id']}", json={"locked": True})
            await c.post("/api/v1/orders", json={
