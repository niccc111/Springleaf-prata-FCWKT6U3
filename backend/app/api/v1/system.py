...
from app.api.deps import AdminOnly, DispatcherOrAdmin, SessionDep
...
async def adapter_config(user: DispatcherOrAdmin) -> dict[str, Any]:
    return {
...
async def summary(session: SessionDep, user: DispatcherOrAdmin) -> dict[str, Any]:
    return {
...
async def ingest_oms(
    ...
    stats = await oms_adapter.process_batch(session, events_batch, acting_user=admin.user_id)
...
async def ingest_fms(
    ...
    stats = await fms_adapter.process_batch(session, events_batch, acting_user=admin.user_id)
    await session.commit()
    return stats
